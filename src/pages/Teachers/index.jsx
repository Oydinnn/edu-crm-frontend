...
export default function TeachersPage() {
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      // ✅ MUHIM: groups ni to'g'ri formatda yuborish
      // Backend JSON string kutadi yoki array property sifatida
      if (selectedGroups.length > 0) {
        // Variant 1: JSON string sifatida
        formData.append("groups", JSON.stringify(selectedGroups));
        
        // Variant 2: Array property sifatida (agar backend array qabul qilsa)
        // selectedGroups.forEach((id) => formData.append("groups", id));
      }

      // ❌ Content-Type ni qo'lda qo'shmang! Browser o'zi to'g'ri qo'shadi
      const res = await api.patch(`/teachers/${editingTeacherId}`, formData, {
        // headers: { "Content-Type": "multipart/form-data" }, // BU QATORNI O'CHIRING!
      });
      ...
      if (res?.data?.success) {
        ...
        // Agar success bo'lmasa lekin data kelsa
        await getTeacher();
        setIsOpen(false);
        setIsEditMode(false);
        resetForm();
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              value={form.password}
              onChange={handleFormChange}
              placeholder={isEditMode ? "Parolni o'zgartirishni istasangiz kiriting" : "Parolni kiriting"}
              className="w-full border border-gray-300 p-2.5 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1f39a1]"
            />
          </div> */}
