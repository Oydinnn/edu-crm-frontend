...
export default function AcceptedHomework() {
  ...
  // Load all accepted students list and detail result
  useEffect(() => {
    ...
    async function loadAllData() {
      ...
      try {
        ...
        const seen = new Set();
        const acceptedStudents = allData.filter((student) => {
          ...
          acceptedStudents.map(async (student) => {
            try {
              const res = await api.get(
                `/group/${groupId}/homework/${homeworkId}/results/${student.id}?status=ACCEPTED`
              );
        ...
        if (studentId) {
          ...
          setExpandedStudentId(acceptedStudents[0].id);
        }
      } catch (err) {
        console.error("Error loading accepted students details data", err);
      } finally {
        setLoading(false);
      }
    }
      ...
      <div className="w-full lg:w-3/5 mr-auto ml-0">
        ...
        {students.length > 0 ? (
          ...
          <div className="py-16 text-center text-sm text-gray-400 bg-white rounded-2xl border border-gray-100 shadow-sm font-sans">
            Qabul qilingan uyga vazifalar mavjud emas.
          </div>
        )}
      </div>
    </div>
