...
export default function HomeworkPending() {
  ...
  const incomingStatus = location.state?.status || "pending";
  ...
  const files = parseFiles(studentResult?.file);
  const hasFile = files.length > 0;
  const currentStatus = studentResult?.status || incomingStatus;
  const statusMeta = statusConfig[currentStatus] || statusConfig.pending;
  const isReviewOnly = currentStatus === "accepted" || currentStatus === "rejected" || currentStatus === "not_done";
  const homeworkResult = studentResult?.homeworkResult || null;
  const examinerName =
    homeworkResult?.teachers?.full_name ||
    `${homeworkResult?.users?.first_name || ""} ${homeworkResult?.users?.last_name || ""}`.trim() ||
    "—";
  const resultTitle = homeworkResult?.title || "—";
  const resultGrade = homeworkResult?.grade ?? "—";
          ...
          <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition-all hover:shadow-md">
            ...
            <div className="mb-6 grid grid-cols-3 gap-4 rounded-xl bg-gray-50/50 p-4 border border-gray-100/50">
              ...
              <div>
                ...
                <span className={`inline-flex items-center rounded-md px-2.5 py-0.5 text-xs font-bold border ${statusMeta.className}`}>
                  {statusMeta.label}
                </span>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition-all hover:shadow-md">
              <div className="mb-6 flex items-start gap-3 rounded-xl bg-blue-50/50 p-4 text-sm text-[#1e40af] border border-blue-100/50 leading-normal">
                ...
                <span className="font-medium">
                  Bu vazifa tekshirilgan. Quyida topshirilgan fayllar va holati ko‘rsatilgan.
                </span>
              </div>

              <div className="grid gap-4 sm:grid-cols-3">
                <div className="rounded-xl bg-gray-50/70 p-4 border border-gray-100/50">
                  <span className="block text-xs font-bold text-gray-400 mb-1">Ball:</span>
